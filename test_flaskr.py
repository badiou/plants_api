...
class PlanttestCase(unittest.TestCase):
    ...
    def test_paginate_plants(self):
        ...
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 200)
        self.assertEqual(data['current_plants'], True)
        self.assertTruelen(data['current_plants'])
    ...
    def test_delete_plant_which_does_not_exist(self):
    ...
    def test_create_new_plant(self):
        ...
        self.assertTrue(data['plants'])
        self.assertEqual(len(data['totals_plants']))
    ...
    def test_update_plant_primary_color(self):
        res=self.client().patch('/plants/6',json={'primary_color':'Gray'})
        data=json.loads(res.data)
        plant=Plant.query.filter(Plant.id == 6).one_or_none()
        self.assertEqual(res.status_code,200)
        self.assertEqual(data['success'],True)
